...
class CommonCommonView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return new Column(
      children: <Widget>[
        ExtendedText(
          "'映射和这个 PODO 文件之间没有映射。\$issue甚至\$实体名称也不匹配。@我知道 我知道。 我们还没有完成。\$我们必须\$将这些类成员映射到 json 对象。为此我们需要创建一个 factory 方法。根据 Dart 文档，我们在实现一个构造函数时使用 factory 关键字时，这个构造函数不会总是创建其类的新实例，而这正是我们现在所需要的'",
